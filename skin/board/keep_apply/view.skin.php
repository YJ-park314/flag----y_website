...
<?php 
    ...
    $vis_desc = '깃발 보관관리 신청내역입니다.';
    include_once(G5_THEME_PATH.'/component/visual.php');
?>

<!-- 게시물 읽기 시작 { -->
<article id="bo_v" class="apply wrap">
    <div class="tit-area">
        <p class="bo-tit"><?php echo $board['bo_subject'] ?></p>

        <!-- 게시물 상단 버튼 시작 { -->
        <div id="bo_v_top">
            <?php ob_start(); ?>
            <ul>
                <!-- <?php if ($reply_href) { ?><li><a href="<?php echo $reply_href ?>" class="btn-ud">답변</a></li><?php } ?> -->
                <?php if ($update_href) { ?><li><a href="<?php echo $update_href ?>" class="btn-ud">수정</a></li><?php } ?>
                <?php if ($delete_href) { ?><li><a href="<?php echo $delete_href ?>" class="btn-ud" onclick="del(this.href); return false;">삭제</a></li><?php } ?>
            </ul>
    ...
    <div class="write_div">
        <p class="tit">단체명</p>
        <div><span><?php echo cut_str(get_text($view['wr_subject']), 70); ?></span></div>
    </div>

    <div class="write_div">
        <p class="tit">대표전화</p>
        <div><span><?php echo $view['wr_1']; ?></span></div>
    </div>
    ...
    <div class="write_div">
        <p class="tit">대표핸드폰</p>
        <div><span><?php echo $view['wr_3']; ?></span></div>
    </div>
    ...
    <div class="write_div flex-wrap">
        <p class="tit">주소</p>
        <?php $addr = explode(',', $view['wr_6']); ?>
        <div>
            <span><?php echo $addr[0]; ?></span>
            <span><?php echo $addr[1]; ?></span>
            <span><?php echo $addr[2]; ?></span>
        </div>
    </div>
    ...
    <div class="write_div fit flex flag">
        <p class="tit">보관수량</p>
        <div>
            <p><span>근조기</span> <?php echo $flag[0]; ?> 개</p>
            <p><span>경하기</span> <?php echo $flag[1]; ?> 개</p>
        </div>
    </div>
    ...
    <div class="write_div flex-wrap">
        <p class="tit">담당자 성함</p>
        <div><?php echo $view['wr_8']; ?></div>
    </div>

    <div class="write_div flex-wrap">
        <p class="tit">담당자 이메일</p>
        <div><?php echo $view['wr_9']; ?></div>
    </div>

    <div class="write_div flex-wrap">
        <p class="tit">청구서 수신</p>
        <div><?php echo $view['wr_10']; ?></div>
    </div>

    <div class="write_div flex-wrap">
        <p class="tit">계산서 발행</p>
        <div><?php echo $view['wr_11']; ?></div>
    </div>
    ...
    <a href="<?php echo $list_href.'&sca='.$_GET['sca'] ?>" class="list-btn btn-point" onclick="getBackList(this);">목록으로</a>
