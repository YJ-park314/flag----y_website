...
if(($bo_table == 'qa' || $bo_table == 'setup_apply' || $bo_table == 'keep_apply' || $bo_table == 'design_confirm') && $member['mb_id'] != 'admin') { // 본인글만 출력
    $sql_extra = " and mb_id = '{$member['mb_id']}' ";
    if($bo_table == 'qa') $sql_extra .= " or (wr_num IN (SELECT wr_num FROM g5_write_qa WHERE mb_id = '{$member['mb_id']}') AND mb_id = 'admin') "; // qa는 어드민 답글까지
}

if ($is_search_bbs) {
    $sql = " select distinct wr_parent from {$write_table} where {$sql_search} ".$sql_extra." {$sql_order} limit {$from_record}, $page_rows ";
} else {
    $sql = " select * from {$write_table} where wr_is_comment = 0 ".$sql_extra;
    if(!empty($notice_array))
