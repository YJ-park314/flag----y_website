...
$name = get_text(trim($_POST['mb_name']));
...
$sql = " select count(*) as cnt from {$g5['member_table']} where mb_name = '$name' AND mb_email = '$email' ";
$row = sql_fetch($sql);
if ($row['cnt'] > 1)
    alert('동일한 메일주소가 2개 이상 존재합니다.\\n\\n관리자에게 문의하여 주십시오.');

$sql = " select mb_no, mb_id, mb_name, mb_nick, mb_email, mb_datetime, mb_leave_date from {$g5['member_table']} where mb_name = '$name' and mb_email = '$email' ";
$mb = sql_fetch($sql);
...
$content .= '<a href="'.G5_URL.'" target="_blank" style="display:block;padding:30px 0;background:#484848;color:#fff;text-decoration:none;text-align:center">로그인 하기</a>';
$content .= '</div>';
$content .= '</div>';
...
alert_close($email.' 메일로 회원아이디를 확인할 수 있는 메일이 발송 되었습니다.\\n\\n메일을 확인하여 주십시오.');